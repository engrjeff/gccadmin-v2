...
import { useNewBelievers } from "@/hooks/use-new-believers";
import { cn } from "@/lib/utils";
import type { ConsolidationCreateInputs } from "./schema";

export function NewBelieversPicker() {
  const consoForm = useFormContext<ConsolidationCreateInputs>();

  const isAdmin = useIsAdmin();
  const leaderId = consoForm.watch("networkLeaderId");
  const newBelievers = consoForm.watch("newBelievers");

  const newBelieversOfNetwork = useNewBelievers({ leaderId });

  const [attendeesSearchQuery, setAttendeesSearchQuery] = useState("");
  const [selectedShown, setSelectedShown] = useState(false);

  const selectedAttendees = newBelieversOfNetwork.data?.filter((d) =>
    newBelievers.includes(d.id),
  );

  const unSelectedAttendees = newBelieversOfNetwork.data?.filter(
    (d) =>
      !newBelievers.includes(d.id) &&
  ...
  const handleAttendeesSelection = (attendeeId: string) => {
    const updatedAttendees = newBelievers.includes(attendeeId)
      ? newBelievers.filter((i) => i !== attendeeId)
      : [...newBelievers, attendeeId];

    consoForm.setValue("newBelievers", updatedAttendees);
  };

  const handleSelectAll = () => {
    consoForm.setValue(
      "newBelievers",
      newBelieversOfNetwork.data?.map((d) => d.id) ?? [],
    );
  };

  const handleDeselectAll = () => {
    consoForm.setValue("newBelievers", []);
  };
      ...
      <div className="relative min-h-[300px] rounded-md border">
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <p className="text-center text-muted-foreground text-sm">
            Select a network leader first
          </p>
        </div>
      </div>
    );

  if (newBelieversOfNetwork?.isLoading)
    return (
      <div className="relative min-h-[300px] rounded-md border">
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <Loader2Icon
    ...
    <div className="relative overflow-hidden rounded-md border">
      <div className="sticky top-0 bg-background p-2">
        <div className="relative">
          <Input
            ...
            placeholder={
              newBelieversOfNetwork.data?.length === 0
                ? "No new believers data"
                : "Search here"
            }
            value={attendeesSearchQuery}
            disabled={newBelieversOfNetwork.data?.length === 0}
        ...
        {newBelieversOfNetwork.data?.length === 0 &&
        newBelieversOfNetwork.isSuccess ? (
          <div className="py-2">
            <p className="mt-1 text-center text-muted-foreground text-sm">
              No new believer record found for the selected network leader.
            </p>
          </div>
        ) : null}
      </div>
      ...
      {newBelievers.length ? (
        <section className="mb-6 border-t">
          <div className="flex items-center justify-between border-b bg-muted/20 px-2.5 py-1">
            ...
            >
              {selectedShown ? "Hide" : "View"} Selected ({newBelievers.length})
            </Button>
            <Button
              type="button"
              size="sm"
                ...
                <li key={`attendee-${d.id}`} className="border-b">
                  <button
                    ...
                    className={cn(
                      ...
                      newBelievers.includes(d.id) ? "bg-muted/30" : "",
                    )}
                  >
      ...
      {unSelectedAttendees?.length ? (
        <section className="border-t">
          <div className="flex items-center justify-between border-b bg-muted/20 px-2.5 py-1">
            <h3 className="font-medium text-xs">New Believers</h3>
            <Button
              type="button"
              size="sm"
              variant="link"
